...
def test_gen_data_gamma_model_Heckman_Mnar(nb_iter, size, a, b, c,  rho, seed):
...
def test_gen_data_frechet_model_Heckman_Mnar(nb_iter, size, a, b, c,  rho, seed):
    ...
    Y = scipy.stats.frechet_r(k).rvs(size=(nb_iter, size))
    ...
    f_Y = scipy.stats.frechet_r(k).pdf(T)
    f_C = scipy.stats.expon(scale=1 / lambda_C).pdf(T)

    S_Y = 1 - scipy.stats.frechet_r(k).cdf(T)
    S_C = 1 - scipy.stats.expon(scale=1 / lambda_C).cdf(T)
...
def test_gen_data_gompertz_model_Heckman_Mnar(nb_iter, size, a, b, c,  rho, seed):
    ...
    Y = scipy.stats.gompertz(k).rvs(size=(nb_iter, size))
    ...
    f_Y = scipy.stats.gompertz(k).pdf(T)
    f_C = scipy.stats.expon(scale=1 / lambda_C).pdf(T)

    S_Y = 1 - scipy.stats.gompertz(k).cdf(T)
    S_C = 1 - scipy.stats.expon(scale=1 / lambda_C).cdf(T)
...
def test_gen_data_pareto_model_Heckman_Mnar(nb_iter, size, a, b, c,  rho, seed):
...
def test_gen_data_weibull_model_Heckman_Mnar(nb_iter, size, a, b, c,  rho, seed):
